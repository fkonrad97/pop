...
namespace md
{
    ...
    public:
        ...
        virtual Action on_snapshot(const SnapshotMsg &msg) = 0;

        virtual Action on_increment(const IncrementMsg &msg) = 0;
        ...
        [[nodiscard]] std::uint64_t getLastSeqID() const noexcept { return last_seq_; }
        void setLastSeqID(std::uint64_t seq) { last_seq_ = seq; }
        std::deque<IncrementMsg> &getBuffer() noexcept { return buffer_; }
        const std::deque<IncrementMsg> &getBuffer() const noexcept { return buffer_; }
        void pushToBuffer(const IncrementMsg &update) { buffer_.push_back(update); }
    ...
    private:
        ...
        std::deque<IncrementMsg> buffer_{};
        std::int64_t last_update_id_;
        SyncState state_;
    };
